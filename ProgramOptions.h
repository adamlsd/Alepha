...
namespace Alepha::inline Cavorite  ::detail::  program_options
{
	...
	namespace exports
	{
		enum OptionMode : bool { Strict= true, NonStrict= false };
	...
	namespace impl
	{
		...
		[[nodiscard]] std::vector< std::string > handleOptions( const std::vector< std::string > &, OptionMode, std::function< void () > );
	}
	...
	namespace exports
	{
		...
		template< typename Supplement >
		auto
		handleOptions( const std::vector< std::string > &args, const OptionMode strictness= Strict )
		{
			return impl::handleOptions( args, strictness, usageWrap< T > );
		}

		template< typename Supplement >
		auto
		handleOptions( const int argcnt, const char *const *const argvec, const OptionMode strictness= Strict )
		{
			return handleOptions< T >( { argvec + 1, argvec + argcnt } strictness, usageWrap< T > );
		}

		auto
		handleOptions( const std::vector< std::string > &args, const OptionMode strictness= Strict )
		{
			return handleOptions< ProgramDescription >( args, strictness );
		}

		auto
		handleOptions( const int argcnt, const char *const *const argvec, const OptionMode strictness= Strict )
		{
			return handleOptions< ProgramDescription >( argcnt, argvec, strictness );
		}
	}
}
